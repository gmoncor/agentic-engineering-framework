...
Alcance de las cifras: la libreria agrega cada transcripcion entera y no desglosa por turno ni por subagente. El coste sale de las tarifas de input y de output de cada modelo conocido; los tokens de cache cuentan para el cache hit rate pero no llevan tarifa propia, asi que el coste es una aproximacion util para comparar sesiones entre si, no una factura.
