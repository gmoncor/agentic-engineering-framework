...
- Nota en el README aclarando que `.claude-plugin/plugin.json` (via nativa de plugin de Claude Code) todavia no es funcional: la raiz del repo usa el layout del backend Gemini (`agents/`, `commands/`, `hooks/hooks.json`), incompatible con lo que espera un plugin de Claude Code. Se recomienda seguir usando `npx ... install --backend claude`, que si copia el contenido correcto
...
- La skill compartida de planificacion instruia lanzar la revision de tareas en paralelo entre varios subagentes revisores, contradiciendo el modelo de ejecucion lineal del framework. Se reescribe el paso para revisar cada tarea de forma secuencial, aclarando que solo la auditoria cruzada final necesita el resultado de todas; se anade el archivo a la cobertura del canario de vocabulario para que el patron no vuelva a colarse sin deteccion
