...
**Si arrancas un proyecto nuevo:** Usa las 4 plantillas en orden (INIT1 → INIT2 → INIT3 → INIT4). El resultado es un proyecto con idea clara, estructura definida, plan de ejecucion y entorno de tests listo.

**Si te incorporas a un proyecto existente:** No necesitas hacer las 4. Evalua cuales te faltan:

| Situacion | Que usar |
|-----------|----------|
| No hay documentacion de que hace el proyecto ni para quien | INIT1 — te ayuda a documentar la vision aunque el proyecto ya exista |
| No esta claro como esta organizado tecnicamente (datos, paginas, arquitectura) | INIT2 — analiza el proyecto existente y genera la documentacion tecnica |
| Hay features pendientes pero no hay un plan de en que orden hacerlos | INIT3 — genera un roadmap basado en dependencias y prioridades |
| No hay framework de testing configurado | INIT4 — configura tests, coverage y scripts en una sola sesion |
...
### INIT1 — Vision del Proyecto

**Archivo:** `INIT1_vision_del_proyecto.md`
...
### INIT2 — Planificacion Tecnica

**Archivo:** `INIT2_planificacion_tecnica.md`
...
**Cuando usarla:**
- Despues de tener la vision del proyecto (INIT1) o equivalente
...
**Que necesitas antes:** El documento de vision (INIT1) o un entendimiento claro de que hace el proyecto.
...
### INIT3 — Roadmap de Desarrollo

**Archivo:** `INIT3_roadmap_de_desarrollo.md`
...
**Cuando usarla:**
- Despues de tener la planificacion tecnica (INIT2)
...
**Que necesitas antes:** El documento de planificacion tecnica (INIT2) o equivalente.
...
### INIT4 — Setup de Testing

**Archivo:** `INIT4_setup_testing.md`
...
```
INIT1  →  Define la idea, los usuarios y el MVP
         Resultado: documento de vision

INIT2  →  Define paginas, datos y arquitectura
         Resultado: documento de planificacion tecnica

INIT3  →  Define el orden de construccion por fases
         Resultado: roadmap con tareas y checkboxes

INIT4  →  Configura el entorno de testing
         Resultado: framework de tests listo para usar

Despues de las INIT, usa las plantillas operativas para el dia a dia:
...
2. Evalua que documentacion falta:
   - No hay vision clara?          →  Usa INIT1
   - No hay documentacion tecnica? →  Usa INIT2
   - No hay plan de trabajo?       →  Usa INIT3
   - No hay tests configurados?    →  Usa INIT4
